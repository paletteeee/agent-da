...
_FORMAL_ABLATION_RECEIPT_FIELDS = frozenset({
    ...
    "status", "candidate_id", "authorization_nonce_sha256",
    "control_plane_attestation_sha256",
})
_FORMAL_ABLATION_ATTESTATION_FIELDS = frozenset({
    "control_plane", "environment", "topology", "cleanup", "candidate_seal",
})
...
def _canonical_path_sha256(path: str | Path) -> str:
    ...
    return hashlib.sha256(str(parent / candidate.name).encode("utf-8")).hexdigest()


def _validate_ablation_document(
    value: Any, *, schema: str, fields: set[str]
) -> dict[str, Any]:
    if (
        not isinstance(value, Mapping)
        or set(value) != fields | {"document_sha256"}
        or value.get("schema") != schema
        or not isinstance(value.get("document_sha256"), str)
        or _SHA256.fullmatch(str(value["document_sha256"])) is None
    ):
        raise CollectorError("formal ablation attestation schema is invalid")
    material = {key: item for key, item in value.items() if key != "document_sha256"}
    if hashlib.sha256(canonical_json_bytes(material)).hexdigest() != value["document_sha256"]:
        raise CollectorError("formal ablation attestation hash is invalid")
    return dict(value)


def _validate_formal_ablation_attestations(
    attestations: Mapping[str, Any], *, receipt: Mapping[str, Any],
    candidate_root: Path, formal_out_dir: Path, samples_sha256: str,
    repetitions_sha256: str, aggregate_sha256: str,
    expected_source_commit: str, expected_config_file_sha256: str,
) -> dict[str, dict[str, Any]]:
    """Validate structured facts emitted inside the root-owned control plane."""
    if not isinstance(attestations, Mapping) or set(attestations) != _FORMAL_ABLATION_ATTESTATION_FIELDS:
        raise CollectorError("formal ablation attestation set is incomplete")
    raw_candidate_root = candidate_root.expanduser().absolute()
    if raw_candidate_root.is_symlink() or not raw_candidate_root.is_dir():
        raise CollectorError("formal ablation candidate root is invalid")
    root_metadata = raw_candidate_root.resolve(strict=True).stat()
    if stat.S_IMODE(root_metadata.st_mode) & 0o022:
        raise CollectorError("formal ablation candidate root is not protected")
    environment = _validate_ablation_document(
        attestations["environment"], schema="txnmem-provenance-ablation-environment-v1",
        fields={"schema", "candidate_id", "authorization_nonce_sha256", "isolation_verified",
                "co_tenant_load_detected", "source", "cpu_logical_count", "memory_total_bytes",
                "disk_medium", "toxiproxy_version"},
    )
    topology = _validate_ablation_document(
        attestations["topology"], schema="txnmem-provenance-ablation-topology-v1",
        fields={"schema", "candidate_id", "authorization_nonce_sha256", "transport",
                "environment_document_sha256", "services"},
    )
    cleanup = _validate_ablation_document(
        attestations["cleanup"], schema="txnmem-provenance-ablation-cleanup-v1",
        fields={"schema", "candidate_id", "authorization_nonce_sha256",
                "namespace_observation_count", "namespace_residue_count",
                "timeout_cleanup_attempt_count", "timeout_cleanup_failure_count",
                "all_workers_quiescent"},
    )
    seal = _validate_ablation_document(
        attestations["candidate_seal"], schema="txnmem-provenance-ablation-candidate-seal-v1",
        fields={"schema", "candidate_id", "authorization_nonce_sha256", "candidate_root_sha256",
                "formal_output_sha256", "samples_sha256", "repetitions_sha256", "aggregate_sha256",
                "partial_resume_status", "completed_repetition_count", "completed_operation_sample_count"},
    )
    control = _validate_ablation_document(
        attestations["control_plane"], schema="txnmem-provenance-ablation-control-plane-v1",
        fields={"schema", "trust_boundary", "candidate_id", "authorization_nonce_sha256",
                "candidate_root_sha256", "formal_output_sha256", "source_commit", "config_sha256",
                "config_file_sha256", "environment_document_sha256", "topology_document_sha256",
                "cleanup_document_sha256", "candidate_seal_sha256"},
    )
    candidate_id = receipt.get("candidate_id")
    nonce_hash = receipt.get("authorization_nonce_sha256")
    if (
        not isinstance(candidate_id, str) or _SHA256.fullmatch(candidate_id) is None
        or not isinstance(nonce_hash, str) or _SHA256.fullmatch(nonce_hash) is None
        or candidate_id != hashlib.sha256(
            f"{samples_sha256}\0{repetitions_sha256}\0{nonce_hash}".encode("utf-8")
        ).hexdigest()
    ):
        raise CollectorError("formal ablation protected candidate identity is invalid")
    candidate_root_hash = _canonical_path_sha256(candidate_root)
    output_hash = _canonical_path_sha256(formal_out_dir)
    for document in (environment, topology, cleanup, seal, control):
        if document.get("candidate_id") != candidate_id or document.get("authorization_nonce_sha256") != nonce_hash:
            raise CollectorError("formal ablation attestation identity drifted")
    services = topology.get("services")
    if not isinstance(services, list) or len(services) != 2:
        raise CollectorError("formal ablation service attestation is invalid")
    service_map = {}
    for row in services:
        if not isinstance(row, Mapping) or set(row) != {"role", "version", "image_manifest_digest_sha256"}:
            raise CollectorError("formal ablation service attestation is invalid")
        role, version, image_hash = row.get("role"), row.get("version"), row.get("image_manifest_digest_sha256")
        if role in service_map or role not in {"qdrant", "neo4j"} or not is_registered_service_version(str(role), version) or image_hash != FORMAL_CONTAINER_IMAGE_MANIFEST_DIGESTS.get(str(role)):
            raise CollectorError("formal ablation service identity is unregistered")
        service_map[str(role)] = dict(row)
    if set(service_map) != {"qdrant", "neo4j"}:
        raise CollectorError("formal ablation service roles are incomplete")
    if (
        environment.get("isolation_verified") is not True
        or environment.get("co_tenant_load_detected") is not False
        or environment.get("source") != "protected_host_probe"
        or type(environment.get("cpu_logical_count")) is not int or environment["cpu_logical_count"] <= 0
        or type(environment.get("memory_total_bytes")) is not int or environment["memory_total_bytes"] <= 0
        or environment.get("disk_medium") not in {"ssd", "nvme", "hdd"}
        or not is_registered_service_version("toxiproxy", environment.get("toxiproxy_version"))
        or topology.get("transport") not in {"local_loopback", "container_bridge"}
    ):
        raise CollectorError("formal ablation protected environment is ineligible")
    if (
        cleanup.get("namespace_observation_count") != 180
        or cleanup.get("namespace_residue_count") != 0
        or type(cleanup.get("timeout_cleanup_attempt_count")) is not int
        or cleanup["timeout_cleanup_attempt_count"] < 0
        or cleanup.get("timeout_cleanup_failure_count") != 0
        or cleanup.get("all_workers_quiescent") is not True
    ):
        raise CollectorError("formal ablation cleanup proof is ineligible")
    if (
        seal.get("candidate_root_sha256") != candidate_root_hash
        or seal.get("formal_output_sha256") != output_hash
        or seal.get("samples_sha256") != samples_sha256
        or seal.get("repetitions_sha256") != repetitions_sha256
        or seal.get("aggregate_sha256") != aggregate_sha256
        or seal.get("partial_resume_status") != "sealed_complete"
        or seal.get("completed_repetition_count") != 180
        or seal.get("completed_operation_sample_count") != 6480
    ):
        raise CollectorError("formal ablation candidate seal is incomplete")
    if (
        control.get("trust_boundary") != "root_owned_local_control_plane"
        or control.get("candidate_root_sha256") != candidate_root_hash
        or control.get("formal_output_sha256") != output_hash
        or control.get("source_commit") != expected_source_commit
        or control.get("config_sha256") != _formal_ablation_config_sha256()
        or control.get("config_file_sha256") != expected_config_file_sha256
        or control.get("environment_document_sha256") != environment["document_sha256"]
        or control.get("topology_document_sha256") != topology["document_sha256"]
        or control.get("cleanup_document_sha256") != cleanup["document_sha256"]
        or control.get("candidate_seal_sha256") != seal["document_sha256"]
        or topology.get("environment_document_sha256") != environment["document_sha256"]
        or receipt.get("control_plane_attestation_sha256") != control["document_sha256"]
        or receipt.get("environment_attestation_sha256") != environment["document_sha256"]
        or receipt.get("topology_attestation_sha256") != topology["document_sha256"]
    ):
        raise CollectorError("formal ablation control-plane binding is invalid")
    return {key: dict(value) for key, value in attestations.items()}
...
def validate_formal_ablation_candidate(
    ...
    aggregate_sha256 = hashlib.sha256(canonical_json_bytes(aggregate) + b"\n").hexdigest()
    validated_attestations = _validate_formal_ablation_attestations(
        attestations, receipt=receipt, candidate_root=Path(candidate_root),
        formal_out_dir=Path(formal_out_dir), samples_sha256=receipt["samples_sha256"],
        repetitions_sha256=receipt["repetitions_sha256"], aggregate_sha256=aggregate_sha256,
        expected_source_commit=expected_source_commit,
        expected_config_file_sha256=expected_config_file_sha256,
    )
    if (
        receipt.get("namespace_residue_count") != validated_attestations["cleanup"]["namespace_residue_count"]
        or receipt.get("timeout_cleanup_failure_count") != validated_attestations["cleanup"]["timeout_cleanup_failure_count"]
    ):
        raise CollectorError("formal ablation cleanup receipt drifted")
...
def promote_formal_ablation_candidate(
    ...
    if (
        ...
        raise CollectorError("formal ablation promotion registry is not protected")
    candidate_id = str(validated["candidate_id"])
    marker_payload = canonical_json_bytes({
        "schema": "txnmem-provenance-ablation-promotion-consumption-v1",
        "candidate_id": candidate_id,
        "authorization_nonce_sha256": validated["authorization_nonce_sha256"],
        "formal_output_sha256": _canonical_path_sha256(target),
        "candidate_seal_sha256": validated["attestations"]["candidate_seal"]["document_sha256"],
    }) + b"\n"
    try:
        marker_fd = os.open(
            registry / f"{candidate_id}.json",
        ...
        raise CollectorError("formal ablation candidate was already consumed") from exc
    try:
    ...
    try:
        ...
        attestation_bytes = canonical_json_bytes(validated["attestations"]) + b"\n"
        aggregate_bytes = canonical_json_bytes(validated["aggregate"]) + b"\n"
        sample_bytes = b"".join(canonical_json_bytes(dict(row)) + b"\n" for row in samples)
        repetition_bytes = b"".join(canonical_json_bytes(dict(row)) + b"\n" for row in repetitions)
        manifest = {
            key: value for key, value in validated.items()
            if key not in {"aggregate", "attestations"}
        }
        manifest["files"] = {
            "samples.jsonl": hashlib.sha256(sample_bytes).hexdigest(),
            "repetitions.jsonl": hashlib.sha256(repetition_bytes).hexdigest(),
            "aggregate.json": hashlib.sha256(aggregate_bytes).hexdigest(),
            "attestations.json": hashlib.sha256(attestation_bytes).hexdigest(),
        }
        files = {
            ...
            "attestations.json": attestation_bytes,
...
def verify_promoted_formal_ablation(out_dir: str | Path) -> dict[str, Any]:
    """Recompute the complete raw → aggregate → manifest hash closure."""
    root = Path(out_dir).expanduser().absolute().resolve(strict=True)
    if root.is_symlink() or not root.is_dir():
        raise CollectorError("formal ablation output is invalid")
    manifest = _load_ablation_json(root / "manifest.json")
    if not isinstance(manifest, Mapping) or not isinstance(manifest.get("files"), Mapping):
        raise CollectorError("formal ablation manifest is invalid")
    expected_files = {"samples.jsonl", "repetitions.jsonl", "aggregate.json", "attestations.json"}
    if set(manifest["files"]) != expected_files:
        raise CollectorError("formal ablation manifest file closure is invalid")
    for name in expected_files:
        raw = (root / name).read_bytes()
        if manifest["files"].get(name) != hashlib.sha256(raw).hexdigest():
            raise CollectorError("formal ablation promoted file hash mismatch")
    samples = _load_ablation_jsonl(root / "samples.jsonl")
    repetitions = _load_ablation_jsonl(root / "repetitions.jsonl")
    recomputed = aggregate_ablation(
        {"samples": samples, "repetitions": repetitions},
        bootstrap_repetitions=int(FORMAL_ABLATION_CONFIG["bootstrap_repetitions"]),
        seed=int(FORMAL_ABLATION_CONFIG["bootstrap_seed"]),
        require_formal_contract=True,
    )
    aggregate = _load_ablation_json(root / "aggregate.json")
    if canonical_json_bytes(recomputed) != canonical_json_bytes(aggregate):
        raise CollectorError("formal ablation aggregate recomputation mismatch")
    aggregate_hash = hashlib.sha256((root / "aggregate.json").read_bytes()).hexdigest()
    if manifest.get("aggregate_sha256") != aggregate_hash:
        raise CollectorError("formal ablation aggregate manifest binding is invalid")
...
def _require_control_plane_file(path: Path) -> Path:
    """Require a local controller-owned immutable input (not a signature)."""
    raw = path.expanduser().absolute()
    if raw.is_symlink() or not raw.is_file():
        raise CollectorError("formal ablation control-plane file is unavailable")
    ...
    if (
        ...
        raise CollectorError("formal ablation control-plane file is not protected")
    return resolved


def _require_sealed_candidate_file(path: Path, candidate_root: Path) -> Path:
    raw_root = candidate_root.expanduser().absolute()
    raw = path.expanduser().absolute()
    if raw_root.is_symlink() or raw.is_symlink() or not raw.is_file():
        raise CollectorError("formal ablation candidate file is unavailable")
    root = raw_root.resolve(strict=True)
    resolved = raw.resolve(strict=True)
    try:
        resolved.relative_to(root)
    except ValueError as exc:
        raise CollectorError("formal ablation candidate file escaped sealed root") from exc
    metadata = resolved.stat()
    if metadata.st_uid not in {os.geteuid(), FORMAL_RUNNER_UID} or stat.S_IMODE(metadata.st_mode) != 0o400:
        raise CollectorError("formal ablation candidate file is not sealed")
    return resolved
...
def _formal_ablation_cli(
    ...
    receipt_path = _require_control_plane_file(args.receipt)
    attestation_path = _require_control_plane_file(args.attestations)
    sample_path = _require_sealed_candidate_file(args.samples, args.candidate_root)
    repetition_path = _require_sealed_candidate_file(args.repetitions, args.candidate_root)
    receipt = _load_ablation_json(receipt_path)
    attestations = _load_ablation_json(attestation_path)
    evidence = {
        "samples": _load_ablation_jsonl(sample_path),
        "repetitions": _load_ablation_jsonl(repetition_path),
    }
    if args.action == "ablation-validate":
        ...
        validated = validate_formal_ablation_candidate(
            ...
            formal_out_dir=args.formal_out_dir,
...
def _require_integrated_lifecycle_fault(value: Any) -> _IntegratedLifecycleFault:
    if type(value) is not _IntegratedLifecycleFault:
        raise CollectorError(
            "integrated lifecycle fault must be an exact enum member"
        )
    return value
...
def _require_candidate_seal_receipt(
    completion_receipt: Any,
) -> dict[str, Any]:
    """Reject absent/partial runner receipts before mutating candidate bytes."""

    if (
        not isinstance(completion_receipt, Mapping)
        or set(completion_receipt) != _MATERIAL_FIELDS
        or completion_receipt.get("schema")
        != "txnmem-provenance-candidate-attestation-material-v1"
    ):
        raise CollectorError("candidate completion receipt is invalid")
    normalized = dict(completion_receipt)
    if any(
        type(normalized.get(field)) is not int or normalized[field] <= 0
        for field in (
            "matrix_cell_count",
            "repetition_count",
            "operation_sample_count",
    ...
    for field in (
        ...
        if (
            type(normalized.get(field)) is not str
            or _SHA256.fullmatch(normalized[field]) is None
        ):
            raise CollectorError("candidate completion receipt is invalid")
    if (
        type(normalized.get("candidate_bundle_id")) is not str
        or re.fullmatch(
            r"diagnostic-vector_graph-[0-9a-f]{16}-[0-9a-f]{16}",
            normalized["candidate_bundle_id"],
        )
        is None
        or not isinstance(normalized.get("observed_service_versions"), Mapping)
        or set(normalized["observed_service_versions"])
...
def _seal_candidate_tree(
    ...
    *,
    ...
    for value, label in (
        ...
        if type(value) is not int or value < 0:
            raise CollectorError(f"{label} is invalid")
    validated_receipt = _require_candidate_seal_receipt(completion_receipt)
    try:
        receipt_raw = canonical_json_bytes(validated_receipt)
    except (TypeError, ValueError) as exc:
...
def _write_collector_completion_record(
    store: FormalStore,
    name: str,
    payload: Any,
) -> Path:
    """Write only a real, present collector completion mapping."""

    if not isinstance(store, FormalStore) or not isinstance(payload, Mapping):
        raise CollectorError("collector completion evidence is unavailable")
    store.write_json_exclusive(name, payload=dict(payload), mode=0o600)
...
def _integrated_lifecycle_config() -> dict[str, Any]:
    return {
...
def _integrated_lifecycle_pidfds() -> dict[int, str]:
    observed: dict[int, str] = {}
...
def _integrated_lifecycle_credentials(pid: int) -> dict[str, Any]:
    try:
        ...
        raise CollectorError(
            "integrated lifecycle process credentials are unavailable"
        ) from None
    rows: dict[str, str] = {}
    ...
    try:
        uids = [int(value) for value in rows["Uid"].split()]
        gids = [int(value) for value in rows["Gid"].split()]
        groups = [int(value) for value in rows.get("Groups", "").split()]
    except (KeyError, ValueError):
        raise CollectorError(
            "integrated lifecycle process credentials are malformed"
        ) from None
    return {"uids": uids, "gids": gids, "groups": groups}


def _integrated_lifecycle_observed_start_ticks(pid: int) -> str | None:
    try:
        ...
        raise CollectorError(
            "integrated lifecycle process identity is unavailable"
        ) from None
    closing_parenthesis = stat_line.rfind(")")
    fields = stat_line[closing_parenthesis + 2 :].split()
    if closing_parenthesis < 0 or len(fields) <= 19 or not fields[19].isdigit():
        raise CollectorError(
            "integrated lifecycle process identity is malformed"
        )
    return fields[19]


def _integrated_lifecycle_start_ticks(pid: int) -> str:
    observed = _integrated_lifecycle_observed_start_ticks(pid)
    if observed is None:
        raise CollectorError(
            "integrated lifecycle process identity is unavailable"
        )
...
def _signal_integrated_lifecycle_identities(
    identities: Mapping[int, str], signal_number: int
) -> int:
    """Signal only exact recorded PID/start identities through pidfds."""

    expected = dict(identities)
    if not expected or any(
        type(pid) is not int
        or pid <= 0
        or type(start_ticks) is not str
        or not start_ticks.isdigit()
        for pid, start_ticks in expected.items()
    ):
        raise CollectorError(
            "integrated lifecycle recorded identity is invalid"
        )
    opened: list[tuple[int, int]] = []
    primary_failure: BaseException | None = None
    primary_traceback = None
    ...
    try:
        for pid in sorted(expected):
            try:
                ...
                if exc.errno == errno.ESRCH:
                    continue
                raise CollectorError(
                    "integrated lifecycle pidfd binding failed"
                ) from None
            opened.append((pid, descriptor))
        for pid, _descriptor in opened:
            if _integrated_lifecycle_start_ticks(pid) != expected[pid]:
                raise CollectorError(
        ...
        for _pid, descriptor in opened:
            try:
                ...
                if exc.errno != errno.ESRCH:
                    raise CollectorError(
                        "integrated lifecycle pidfd signal failed"
                    ) from None
    except BaseException as exc:
        primary_failure = exc
        primary_traceback = exc.__traceback__
    ...
    if close_failures:
        raise CollectorError(
            "integrated lifecycle pidfd cleanup failed"
        ) from None
    return delivered


@dataclass(frozen=True)
class _IntegratedLifecycleGuardOwner:
    guard: Any
    owner_pid: int
    ...
    def __post_init__(self) -> None:
        ...
        if not callable(getattr(self.guard, "deactivate", None)):
            raise CollectorError(
                "integrated lifecycle guard owner is invalid"
            )
    ...
    def deactivate_after_quiescence(
        ...
        if not callable(quiescence_check):
            raise CollectorError(
                "integrated lifecycle guard quiescence check is invalid"
            )
        quiescence_check()
        self.guard.deactivate()


_INTEGRATED_WORKER_STATE_FIELDS = frozenset(
    {
        "schema",
        "runner_pid",
        "runner_start_ticks",
...
def _normalize_integrated_lifecycle_worker_state(value: Any) -> dict[str, Any]:
    if (
        not isinstance(value, Mapping)
        or set(value) != _INTEGRATED_WORKER_STATE_FIELDS
        or value.get("schema")
        != "txnmem-integrated-lifecycle-worker-state-v1"
        or type(value.get("runner_pid")) is not int
        or value["runner_pid"] <= 0
        or type(value.get("descendant_pid")) is not int
        or value["descendant_pid"] <= 0
        or value["runner_pid"] == value["descendant_pid"]
        or type(value.get("runner_start_ticks")) is not str
        or not value["runner_start_ticks"].isdigit()
        or type(value.get("descendant_start_ticks")) is not str
        or not value["descendant_start_ticks"].isdigit()
    ):
        raise CollectorError(
...
def _send_integrated_lifecycle_worker_state(
    channel: socket.socket,
    state: Mapping[str, Any],
    receipt_fd: int,
) -> None:
    normalized = _normalize_integrated_lifecycle_worker_state(state)
    if (
        not isinstance(channel, socket.socket)
        or type(receipt_fd) is not int
        or receipt_fd < 0
    ):
        raise CollectorError(
    ...
    descriptors = array.array("i", [receipt_fd])
    try:
        ...
        raise CollectorError(
            "integrated lifecycle worker state transfer failed"
        ) from None
    if sent != len(payload):
        raise CollectorError(
            "integrated lifecycle worker state transfer failed"
...
def _harvest_integrated_lifecycle_received_fds(
    ...
    received_owners: list[_IntegratedLifecycleFdOwner],
    owner_group: _IntegratedLifecycleFdOwnerGroup,
) -> None:
...
def _receive_integrated_lifecycle_worker_state(
    ...
    *,
    ...
    if (
        ...
        or not isinstance(resources, _IntegratedLifecycleResources)
        or resources.receipt_owner.has_owner
    ):
        raise CollectorError(
            "integrated lifecycle worker channel is invalid"
        )
    ...
    try:
        ...
        payload, ancillary, message_flags, _address = (
            _recv_integrated_lifecycle_message_owned(
                channel,
                flags=flags,
                received_owners=received_owners,
                resources=resources,
            )
        )
    ...
    if primary_failure is not None:
        raise primary_failure.with_traceback(primary_traceback)
    if normalized is None or resources.receipt_owner.descriptor is None:
...
def _read_integrated_lifecycle_absent_receipt(
    ...
    try:
        while True:
            ...
            if remaining <= 0.0:
                raise CollectorError(
                    "integrated lifecycle receipt EOF timed out"
                )
            ...
            if len(payload) > 65536:
                raise CollectorError(
                    "integrated lifecycle receipt is oversized"
                )
        if payload:
            raise CollectorError(
                "integrated lifecycle completion receipt was not absent"
            )
    except BaseException as exc:
        primary_failure = exc
        primary_traceback = exc.__traceback__
...
def _integrated_lifecycle_expected_owner(
    relative: Path,
    *,
    controller_uid: int,
    runner_uid: int,
    ...
    if relative == runner_owned_relative or runner_owned_relative in relative.parents:
        return runner_uid
    return controller_uid


def _inspect_integrated_lifecycle_tree_fd(
    directory_fd: int,
    *,
    relative: Path,
    controller_uid: int,
    ...
    expected_owner = _integrated_lifecycle_expected_owner(
        relative,
    ...
    if (
        ...
        raise CollectorError(
            "integrated lifecycle cleanup owner or type changed"
        )
    ...
    try:
        ...
        raise CollectorError(
            "integrated lifecycle cleanup enumeration failed"
        ) from None
    for name in names:
        if (
            ...
            raise CollectorError(
                "integrated lifecycle cleanup entry is invalid"
            )
        child_relative = (
            Path(name) if relative == Path(".") else relative / name
        )
        expected_child_owner = _integrated_lifecycle_expected_owner(
            child_relative,
            controller_uid=controller_uid,
            runner_uid=runner_uid,
            runner_owned_relative=runner_owned_relative,
        ...
        try:
            ...
            raise CollectorError(
                "integrated lifecycle cleanup inspection failed"
            ) from None
        if metadata.st_uid != expected_child_owner:
            raise CollectorError(
                "integrated lifecycle cleanup owner or type changed"
            )
        if stat.S_ISDIR(metadata.st_mode):
            ...
            try:
                ...
                node = _inspect_integrated_lifecycle_tree_fd(
                    child_fd,
                ...
                os.close(child_fd)
            children.append({"name": name, **node})
        elif stat.S_ISREG(metadata.st_mode):
            file_fd = os.open(
                name,
                os.O_RDONLY
            ...
            try:
                ...
                if (
                    ...
                    or int(opened.st_ino) != int(metadata.st_ino)
                    or opened.st_uid != expected_child_owner
                ):
                    raise CollectorError(
                        "integrated lifecycle cleanup identity changed"
                    )
            ...
            raise CollectorError(
                "integrated lifecycle cleanup found a special file"
            )
    return {
        "kind": "directory",
        "device": int(directory_metadata.st_dev),
        "inode": int(directory_metadata.st_ino),
        "owner_uid": int(directory_metadata.st_uid),
        "children": children,
    }
...
def _close_integrated_lifecycle_fd_object(file_object: io.FileIO) -> None:
    file_object.close()
...
@dataclass
class _IntegratedLifecycleFdOwnerGroup:
    owners: list[_IntegratedLifecycleFdOwner] = field(default_factory=list)

    def owner_for_descriptor(
        self, descriptor: int
    ) -> _IntegratedLifecycleFdOwner | None:
        matches = [
            owner
            for owner in self.owners
            if not owner.closed and owner.initial_descriptor == descriptor
        ]
        if len(matches) > 1:
            raise CollectorError(
                "integrated lifecycle descriptor ownership is ambiguous"
            )
        return None if not matches else matches[0]

    def adopt_descriptor(self, descriptor: int) -> _IntegratedLifecycleFdOwner:
        if self.owner_for_descriptor(descriptor) is not None:
            raise CollectorError(
    ...
    def close_retained(self) -> list[_IntegratedLifecycleCapturedFailure]:
        failures: list[_IntegratedLifecycleCapturedFailure] = []
        for owner in self.owners:
            try:
                owner.close_retained()
            except BaseException as exc:
                failures.append(
                    _IntegratedLifecycleCapturedFailure(
                        exception=exc,
                        traceback=exc.__traceback__,
                    )
                )
...
class _IntegratedLifecycleReceiptOwner:
    fd_owner: _IntegratedLifecycleFdOwner | None = None

    @property
    def has_owner(self) -> bool:
        return self.fd_owner is not None

    @property
    def descriptor(self) -> int | None:
        if self.fd_owner is None or self.fd_owner.closed:
            return None
        return self.fd_owner.borrow()

    def adopt_owner(self, owner: _IntegratedLifecycleFdOwner) -> None:
        if (
            not isinstance(owner, _IntegratedLifecycleFdOwner)
            or self.fd_owner is not None
            or owner.closed
        ):
            raise CollectorError(
                "integrated lifecycle receipt ownership is invalid"
    ...
    def close_retained(self) -> None:
        if self.fd_owner is not None:
            self.fd_owner.close_retained()
...
class _IntegratedLifecycleResources:
    ...
    root_absent_before_attempt: bool = False
    ...
    fd_owners: _IntegratedLifecycleFdOwnerGroup = field(
        default_factory=_IntegratedLifecycleFdOwnerGroup
    )
    ...
    deferred_failures: list[_IntegratedLifecycleCapturedFailure] = field(
        default_factory=list
    )
...
def _close_integrated_lifecycle_directory_fd_once(
    descriptor: int,
    *,
    label: str,
) -> None:
    if type(descriptor) is not int or descriptor < 0 or not label:
        raise CollectorError(
...
def _bind_integrated_lifecycle_root(
    ...
    try:
        ...
        if root_fd is not None:
            try:
                _close_integrated_lifecycle_directory_fd_once(
                    root_fd,
    ...
    if primary_failure is not None:
        if close_failure is not None:
            resources.defer_failure(close_failure)
        raise primary_failure.with_traceback(primary_traceback)
    if close_failure is not None:
        raise close_failure
...
def _acquire_integrated_lifecycle_root(
    ...
    try:
        ...
        if (
            ...
            and not resources.root_name_collision
        ):
            try:
                _bind_integrated_lifecycle_root(resources, parent_fd)
            except FileNotFoundError:
                ...
                resources.defer_failure(reconciliation_failure)
    finally:
...
def _remove_integrated_lifecycle_tree(
    ...
    *,
    ...
    if target.parent != parent or not target.name:
        raise CollectorError("integrated lifecycle cleanup identity changed")
    if (
        not isinstance(runner_owned_relative, Path)
        or runner_owned_relative.is_absolute()
        or runner_owned_relative == Path(".")
    ...
    try:
        ...
        if (
            not stat.S_ISDIR(parent_metadata.st_mode)
            or int(parent_metadata.st_dev) != expected_parent_device
            or int(parent_metadata.st_ino) != expected_parent_inode
            or parent_metadata.st_uid != controller_uid
        ):
            raise CollectorError(
                "integrated lifecycle cleanup parent identity changed"
        ...
        manifest = _inspect_integrated_lifecycle_tree_fd(
            root_fd,
            relative=Path("."),
            controller_uid=controller_uid,
            runner_uid=runner_uid,
...
def _finalize_integrated_lifecycle_resources(
    ...
    *,
    ...
    for deferred in resources.deferred_failures:
        resources.record_failure(
            deferred.exception,
            deferred.traceback,
        )
    resources.deferred_failures.clear()
    ...
    for field_name in ("parent_channel", "worker_channel"):
        try:
            ...
            resources.record_failure(exc)
    for captured in resources.fd_owners.close_retained():
        resources.record_failure(
            captured.exception,
            captured.traceback,
        )
...
def _run_protected_linux_integrated_lifecycle_body(
    *,
    ...
    def worker_main() -> None:
        ...
        try:
            if worker_channel.recv(1) != b"G":
                raise CollectorError(
                    "integrated lifecycle worker gate failed"
                )
            ...
            while (
                (not pointer_path.is_file() or not precommit_marker.is_file())
                and time.monotonic() < deadline
            ):
                if child.process.poll() is not None:
                    raise CollectorError(
                        "integrated lifecycle runner exited before pointer"
                    )
                time.sleep(0.01)
            if not pointer_path.is_file() or not precommit_marker.is_file():
                raise CollectorError("integrated lifecycle pointer timed out")
            store = FormalStore(workspace.candidate)
            ...
            if (
                ...
                raise CollectorError("integrated lifecycle pointer is invalid")
            object_id = pointer.get("object_id")
            if (
                type(object_id) is not str
                or store.entry_kind(
                    "bundle_objects", object_id, "COMPLETED.json"
                )
                != "file"
            ):
                raise CollectorError(
                    "integrated lifecycle object completion is unavailable"
                )
            if external_completion.exists():
                raise CollectorError(
            ...
            if child._signal_formal_inventory(
                ...
                raise CollectorError(
                    "integrated lifecycle resistant signal was not delivered"
                )
            time.sleep(0.05)
            if (
                child.process.poll() is not None
                or child._formal_group_members() != exact_inventory
                or _integrated_lifecycle_pidfds() != pidfds_before_term
            ):
                raise CollectorError(
                    "integrated lifecycle descendant did not resist termination"
                )
            pidfds_at_drift = _integrated_lifecycle_pidfds()
            drifted = dict(exact_inventory)
            drift_pid = max(drifted)
            drifted[drift_pid] = str(int(drifted[drift_pid]) + 1)
            try:
                child._signal_formal_inventory(drifted, signal.SIGTERM)
            except CollectorError:
                pass
            else:
                raise CollectorError(
                    "integrated lifecycle pidfd drift was not rejected"
                )
            if (
                child.process.poll() is not None
                or child._formal_group_members() != exact_inventory
                or _integrated_lifecycle_pidfds() != pidfds_at_drift
            ):
                raise CollectorError(
    ...
    try:
        ...
        runner_inventory = {
            ...
            int(worker_state["descendant_pid"]): str(
                worker_state["descendant_start_ticks"]
            ),
        }
        runner_groups = {
            pid: _read_process_group_identity(pid, command)
            for pid in runner_inventory
        }
        leaders = [
            pid
            for pid, identity in runner_groups.items()
            if identity["pgid"] == pid and identity["sid"] == pid
        ]
        if len(leaders) != 1:
            raise CollectorError(
                "integrated lifecycle runner identity is ambiguous"
            )
        runner_pid = leaders[0]
        descendants = set(runner_inventory) - {runner_pid}
        if any(
            runner_groups[pid]["pgid"] != runner_pid
            or runner_groups[pid]["sid"] != runner_pid
        ...
        if _signal_integrated_lifecycle_identities(
            {worker_pid: worker_start}, signal.SIGKILL
        ) != 1:
            raise CollectorError(
                "integrated lifecycle collector parent death was not delivered"
            )
        lifecycle_events.append("collector_parent_death")

        deadline = time.monotonic() + 8.0
        expected_pids = {worker_pid, *runner_inventory}
        while time.monotonic() < deadline:
            for pid in expected_pids - set(reaped_statuses):
                try:
                    waited_pid, status = os.waitpid(pid, os.WNOHANG)
                except ChildProcessError:
                    continue
            ...
            if (
                ...
                and not _formal_uid_processes(FORMAL_RUNNER_UID)
            ):
                break
            time.sleep(0.01)
        if set(reaped_statuses) != expected_pids or any(
            not os.WIFSIGNALED(status)
            or os.WTERMSIG(status) != signal.SIGKILL
            for status in reaped_statuses.values()
        ):
            raise CollectorError(
                "integrated lifecycle actual parent death was not proven"
            )
        _require_formal_uid_processes(FORMAL_RUNNER_UID, expected={})
        lifecycle_events.append("process_quiescence")
        if guard.table_name not in guard._table_names():
            raise CollectorError(
                "integrated lifecycle guard disappeared before quiescence"
            )
        ...
        try:
            ...
            completion_writer_rejected = True
        if (
            not completion_writer_rejected
            or str(completion_failure)
            != "collector completion evidence is unavailable"
            or external_completion_store.entry_kind(
                external_completion_name
            )
            != "missing"
        ):
            raise CollectorError(
                "integrated lifecycle external completion boundary accepted absence"
            )
        lifecycle_events.append("completion_writer_rejected")

        seal_invoked = False
        seal_rejected = False
        seal_failure: CollectorError | None = None
        try:
            seal_invoked = True
        ...
        if (
            not seal_invoked
            or not seal_rejected
            or str(seal_failure) != "candidate completion receipt is invalid"
            or not pointer_path.is_file()
        ):
            raise CollectorError(
                "integrated lifecycle missing receipt reached candidate seal"
            )
        lifecycle_events.append("seal_rejected")
        ...
        promotion_failure: ProvenancePerformanceError | None = None
        promotion_output = lifecycle_root / "formal-output"
        try:
            ...
            promote_provenance_candidate(
                ...
                out_dir=promotion_output,
            )
        except ProvenancePerformanceError as exc:
            promotion_failure = exc
            promotion_rejected = True
        if (
            not promotion_invoked
            or not promotion_rejected
            or str(promotion_failure) != "formal topology omits candidate seal"
            or promotion_output.exists()
            or not pointer_path.is_file()
        ):
            raise CollectorError(
                "integrated lifecycle pointer reached formal promotion"
            )
        lifecycle_events.append("promotion_rejected")
        if external_completion.exists():
            raise CollectorError(
                "integrated lifecycle external completion appeared"
            )
