...
| 5. 并发/micro-witness | `txnmem_interleavings.py` 穷举保持各 agent 局部顺序的所有 linearization；`txnmem_concurrency.py` 提供真实线程锁线性化 smoke harness | 已完成小规模真实线程 harness；仍不等同于生产级跨进程/分布式事务 |
...
1. 已完成三类官方输入的 workflow/API projection replay；下一步是让实际 Agent memory backend 输出本仓库的 canonical event contract。
...
4. 已完成小规模 interleaving 与真实线程锁 smoke harness；下一步是从真实 trace 生成每个 agent 的局部 operation sequence，并运行完整 linearization 检查。
