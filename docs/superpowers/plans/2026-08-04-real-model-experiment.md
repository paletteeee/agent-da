...
- [ ] **Step 1: Write failing tests** for parsing a tool-call response, malformed JSON, HTTP error, and request metadata redaction.
- [ ] **Step 2: Run the focused tests and verify the missing module/API failure.**
- [ ] **Step 3: Implement the standard-library JSON/HTTP client and strict response parser.**
- [ ] **Step 4: Run focused tests and verify all protocol cases pass.**
- [ ] **Step 5: Commit** with `feat: add compatible model protocol client`.
...
- [ ] **Step 1: Write failing tests** for read/write/derive tool dispatch, unknown tool failure, max-step failure, and validated native event output.
- [ ] **Step 2: Run focused tests and verify the missing runner/gateway failure.**
- [ ] **Step 3: Implement the minimal tool schema, gateway dispatch, conversation loop, and structured run report.**
- [ ] **Step 4: Run focused tests and verify derive preserves `source_ids` from the actual call.**
- [ ] **Step 5: Commit** with `feat: add real agent memory tool loop`.
...
- [ ] **Step 1: Write failing tests** for trace-to-instance conversion, variant oracle-match aggregation, raw payload removal, and missing model configuration.
- [ ] **Step 2: Run focused tests and verify the missing evaluator failure.**
- [ ] **Step 3: Implement independent differential evaluation and aggregate-only report generation.**
- [ ] **Step 4: Run focused tests and verify reports contain no raw values/content/arguments.**
- [ ] **Step 5: Commit** with `feat: evaluate native model traces independently`.
...
- [ ] **Step 1: Write failing CLI tests** for manifest validation and aggregate-only output.
- [ ] **Step 2: Run focused CLI tests and verify the missing command failure.**
- [ ] **Step 3: Add `real-model-smoke` CLI and a deterministic fake-compatible model path for local tests.**
- [ ] **Step 4: Run the CLI tests and example; verify no model result is claimed without endpoint configuration.**
- [ ] **Step 5: Commit** with `feat: add real model experiment CLI`.

### Task 5: Realistic task manifest, splits, and failure schedules

**Files:**
- Create: `configs/real_model_tasks.json`
- Modify: `src/txnmem_schedules.py`
- Modify: `docs/remaining_tasks_implementation_zh.md`
- Test: `tests/test_txnmem_real_model.py`

- [ ] **Step 1: Write failing tests** for episode-level split, trigger-based policy/crash schedules, and stable manifest hashes.
- [ ] **Step 2: Run focused tests and verify the missing manifest/schedule behavior.**
- [ ] **Step 3: Add a small public-task manifest schema and schedule predicates without embedding private raw data.**
- [ ] **Step 4: Run focused tests and verify deterministic split and schedule coverage.**
- [ ] **Step 5: Commit** with `docs: define real model task manifest`.
...
- [ ] **Step 1: Run the full test suite and all aggregate-output assertions.**
- [ ] **Step 2: Update paper/documentation to distinguish harness-ready from real-endpoint evidence.**
- [ ] **Step 3: Regenerate DOCX, run unzip/a11y/heading/section audits, and record renderer limitations.**
