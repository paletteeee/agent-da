...
更新时间：2026-08-14。以下状态区分“代码/实验接口已完成”“已完成正式 batch”“仍不足以支撑投稿结论”，避免把路径观察误写成持久状态或原子性证据。
...
| 7 | 并发、跨进程、协议故障 | 验证基础设施已修复；投稿实验阻塞 | 4 类确定性 protocol schedule、5/5 invariant coverage；旧 Toxiproxy 5×30 仅支持四个 non-normal 场景 trigger/toxic/proxy-path 的 30/30 路径观察；新 verifier 在恢复后读取 Qdrant/Neo4j 并 fail-closed 分类 complete/absent/partial/unknown，含 Neo4j commit 后客户端异常回归 | 必须在可用的真实 Qdrant/Neo4j/Toxiproxy 上完成新的 state-verified 5×30；旧结果不支持双存储状态、原子性、可用性或延迟结论 |
...
| 11 | 论文初稿与结果同步 | CCF-A 质量工作稿；非 submission-ready evidence | 正文、6 图、8 表、32 条参考文献和确定性 DOCX 构建链均已同步降级 Toxiproxy 主张 | 仍需新的 state-verified 5×30、选定 venue 后模板适配和正常作者修订 |
| 12 | Claim ledger 与历史结果作废 | 已完成（降级后） | 15 条 active claim、131 个字段断言；Toxiproxy active claim 只绑定 fault/response-path 字段，删除持久状态与 atomicity 断言；claim/artifact/manuscript audit 均要求 current bytes | 新增论文数字或完成 state-verified rerun 时必须同步更新 ledger 并重跑 fail-closed audit |
...
5. 旧单机故障矩阵只支持请求穿过 Toxiproxy、四个 non-normal 场景 trigger/toxic/proxy-path 均为 30/30，以及所记录的客户端 success/error/abort/retry 响应；故障后 Qdrant/Neo4j 状态未被独立核验，不能据此主张 atomicity、availability 或 latency。5-task E2E 仍只作为单机闭环 smoke。
...
- 旧单机 Qdrant/Neo4j/Toxiproxy 5×30 只承担 proxy/fault-response 路径观察；其 post-fault 双存储状态未独立核验。新 verifier 已实现，但本会话没有可用 Docker/真实服务，未生成或冒充 rerun 证据。
...
- Claim audit 当前口径为 15 条正式主张和 131 个字段断言；`results/remaining_tasks/final_status.json`、`results/remaining_tasks/production_evidence_status.json` 和旧 backend fault artifact 仅保留作历史审计，不得再被正文引用为当前状态。
- 本分支产物定位为 CCF-A 质量工作稿，不是 submission-ready evidence。

## Manuscript readiness blockers

1. 在真实 Qdrant/Neo4j/Toxiproxy 上完成新的 state-verified 5×30 rerun，并据实更新证据。
2. 选定 venue 后完成匿名与版式模板适配。
3. 完成正常作者修订。
